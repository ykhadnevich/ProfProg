...
#define ASSERT_EQ(leftValue, rightValue) \
    if (leftValue != rightValue) { \
        std::cout << "Assert failed: " << #leftValue << " (which is equal to " << leftValue << ") is not equal to " << #rightValue << " at " << __FILE__ << ":" << __LINE__ << std::endl; \
        throw std::invalid_argument("Assertion failed"); \
    }

#define ASSERT_NEQ(leftValue, rightValue) \
    if (leftValue == rightValue) { \
        std::cout << "Assert failed: " << #leftValue << " is equal to " << #rightValue << " at " << __FILE__ << ":" << __LINE__ << std::endl; \
        throw std::invalid_argument("Assertion failed"); \
    }
